...
log_path = str(f"{log_dir}/{CONF_LOGFILE}")
...
class rs485:
        ...
        if d_type == "serial":
            ...
            for item in get_conf_serial:
                if item[1] != "":
                    ...
                    logger.info(f"[CONFIG] {CONF_SERIAL} {item[0]} : {item[1]}")
                port_i += 1
            ...
            for item in get_conf_serial_device:
                if item[1] != "":
                    ...
                    logger.info(f"[CONFIG] {CONF_SERIAL_DEVICE} {item[0]} : {item[1]}")
                port_i += 1
        ...
        for p in port:
            try:
                ...
                if ser[p].isOpen():
                    ...
                    logger.info(f"Port {p} : {port[p]}")
                    opened += 1
                else:
                    logger.info(f"시리얼포트가 열려있지 않습니다.[{port[p]}]")
            except serial.serialutil.SerialException:
                logger.info(f"시리얼포트에 연결할 수 없습니다.[{port[p]}]")
        if opened == 0:
            return False
        return ser
    ...
    def connect_socket(self, server, port):
        ...
        try:
            ...
            logger.info(f"소켓에 연결할 수 없습니다.[{e}][{server}:{port}]")
            return False
        soc.settimeout(None)
        return soc
...
class Kocom(rs485):
    ...
    def on_message(self, client, obj, msg):
        ...
        if (
            ...
            if _topic[3] == "log_level":
                ...
                logger.info(f"[From HA]Set Loglevel to {_payload}")
                return
                ...
                logger.info(f"[From HA]{_payload} = {chksum[0]}({chksum[1]})")
        elif not self.kocom_scan:
            self.parse_message(_topic, _payload)
            return
        logger.info(f"Message: {msg.topic} = {_payload}")
    ...
    def homeassistant_device_discovery(self, initial=False, remove=False):
        ...
        if self.wp_elevator:
            ...
            ha_payload = {
                ...
                "pl_on": "on",
                "pl_off": "off",
                "uniq_id": f"{self._name}_wallpad_{DEVICE_ELEVATOR}",
                "device": {
                    "name": "Kocom wallpad",
                    "ids": "kocom_wallpad",
                    "mf": "KOCOM",
        ...
        if self.wp_gas:
            ha_topic = f"{HA_PREFIX}/{HA_SWITCH}/wallpad_{DEVICE_GAS}/config"
            ha_payload = {
                "name": f"{self._name}_wallpad_{DEVICE_GAS}",
                "command_topic": f"{HA_PREFIX}/{HA_SWITCH}/wallpad_{DEVICE_GAS}/set",
                "state_topic": f"{HA_PREFIX}/{HA_SWITCH}/wallpad_{DEVICE_GAS}/state",
                "value_template": f"{{{{ value_json.{DEVICE_GAS} }}}}",
                "ic": "mdi:gas-cylinder",
                "pl_on": "on",
                "pl_off": "off",
                "uniq_id": f"{self._name}_wallpad_{DEVICE_GAS}",
                "device": {
                    "name": "Kocom wallpad",
                    "ids": "kocom_wallpad",
                    "mf": "KOCOM",
            ...
            ha_topic = f"{HA_PREFIX}/{HA_SENSOR}/wallpad_{DEVICE_GAS}/config"
            ha_payload = {
                "name": f"{self._name}_wallpad_{DEVICE_GAS}",
                "state_topic": f"{HA_PREFIX}/{HA_SENSOR}/wallpad_{DEVICE_GAS}/state",
                "value_template": f"{{{{ value_json.{DEVICE_GAS} }}}}",
                "ic": "mdi:gas-cylinder",
                "uniq_id": f"{self._name}_wallpad_{DEVICE_GAS}",
                "device": {
        ...
        if self.wp_fan:
            ...
            ha_payload = {
                "name": f"{self._name}_wallpad_{DEVICE_FAN}",
                ...
                "pl_on": "on",
                "pl_off": "off",
                "percentage_command_topic": f"{HA_PREFIX}/{HA_FAN}/wallpad/speed",
                "percentage_state_topic": f"{HA_PREFIX}/{HA_FAN}/wallpad/state",
                "percentage_value_template": "{{ value_json.speed }}",
                "speed_range_max": 3,
                "uniq_id": f"{self._name}_wallpad_{DEVICE_FAN}",
                "device": {
                    "name": "Kocom wallpad",
                    "ids": "kocom_wallpad",
                    "mf": "KOCOM",
                ...
                if type(r_value) == dict:
                    for sub_device, d_value in r_value.items():
                        if type(d_value) == dict:
                            ha_topic = (
                                f"{HA_PREFIX}/{HA_LIGHT}/{room}_{sub_device}/config"
                            )
                            ha_payload = {
                                "name": f"{self._name}_{room}_{sub_device}",
                                "command_topic": f"{HA_PREFIX}/{HA_LIGHT}/{room}_{sub_device}/set",
                                "state_topic": f"{HA_PREFIX}/{HA_LIGHT}/{room}/state",
                                "state_value_template": "{{ value_json."
                                + str(sub_device)
                                + " }}",
                                "pl_on": "on",
                                "pl_off": "off",
                                "uniq_id": f"{self._name}_{room}_{sub_device}",
                                "device": {
                                    "name": f"Kocom {room}",
                                    "ids": f"kocom_{room}",
                                    "mf": "KOCOM",
            ...
            for room, r_value in self.wp_list[DEVICE_PLUG].items():
                if type(r_value) == dict:
                    for sub_device, d_value in r_value.items():
                        if type(d_value) == dict:
                            ha_topic = (
                                f"{HA_PREFIX}/{HA_SWITCH}/{room}_{sub_device}/config"
                            )
                            ha_payload = {
                                "name": f"{self._name}_{room}_{sub_device}",
        ...
        if self.wp_thermostat:
            for room, r_list in self.wp_list[DEVICE_THERMOSTAT].items():
                if type(r_list) == dict:
                    ha_topic = f"{HA_PREFIX}/{HA_CLIMATE}/{room}/config"
                    ha_payload = {
                        "name": f"{self._name}_{room}_{DEVICE_THERMOSTAT}",
    ...
    def send_to_homeassistant(self, device, room, value):
        ...
        if device == DEVICE_LIGHT:
            self.d_mqtt.publish(f"{HA_PREFIX}/{HA_LIGHT}/{room}/state", v_value)
            logger.info(f"[To HA]{HA_PREFIX}/{HA_LIGHT}/{room}/state = {v_value}")
        elif device == DEVICE_PLUG:
            self.d_mqtt.publish(f"{HA_PREFIX}/{HA_SWITCH}/{room}/state", v_value)
            logger.info(f"[To HA]{HA_PREFIX}/{HA_SWITCH}/{room}/state = {v_value}")
        elif device == DEVICE_THERMOSTAT:
            self.d_mqtt.publish(f"{HA_PREFIX}/{HA_CLIMATE}/{room}/state", v_value)
            logger.info(f"[To HA]{HA_PREFIX}/{HA_CLIMATE}/{room}/state = {v_value}")
        elif device == DEVICE_ELEVATOR:
            ...
            v_value = json.dumps({device: value})
            self.d_mqtt.publish(
                f"{HA_PREFIX}/{HA_SENSOR}/{room}_{DEVICE_GAS}/state", v_value
            )
            logger.info(
                f"[To HA]{HA_PREFIX}/{HA_SENSOR}/{room}_{DEVICE_GAS}/state = {v_value}"
            )
            self.d_mqtt.publish(
                f"{HA_PREFIX}/{HA_SWITCH}/{room}_{DEVICE_GAS}/state", v_value
            )
            logger.info(
                f"[To HA]{HA_PREFIX}/{HA_SWITCH}/{room}_{DEVICE_GAS}/state = {v_value}"
            )
        elif device == DEVICE_FAN:
            self.d_mqtt.publish(f"{HA_PREFIX}/{HA_FAN}/{room}/state", v_value)
            logger.info(f"[To HA]{HA_PREFIX}/{HA_FAN}/{room}/state = {v_value}")
    ...
    def get_serial(self, packet_name, packet_len):
        ...
        while True:
            ...
            if len(packet) >= packet_len:
                ...
                if chksum[0]:
                    self.tick = time.time()
                    logger.debug(f"[From {packet_name}]{packet}")
    ...
    def value_packet(self, p):
        ...
        try:
            ...
            target_device = (
                ...
                and KOCOM_DEVICE.get(p["src_device"]) != None
            ...
            if target_device == DEVICE_THERMOSTAT:
                target_room = (
                    ...
                    if KOCOM_DEVICE.get(p["dst_room"]) != DEVICE_WALLPAD
                    and KOCOM_DEVICE.get(p["src_device"]) != None
            ...
            if target_device == DEVICE_FAN:
                ...
                v["value"] = self.parse_switch(target_device, target_room, p["value"])
            elif target_device == DEVICE_THERMOSTAT:
                v["value"] = self.parse_thermostat(
                    p["value"],
                    self.wp_list[target_device][target_room]["target_temp"]["state"],
                )
    ...
    def set_serial(self, device, room, target, value, cmd="상태"):
        if (time.time() - self.tick) < KOCOM_INTERVAL / 1000:
            return
        if cmd == "상태":
            logger.info(f"[To {self._name}]{device}/{room}/{target} -> {value}")
        elif cmd == "조회":
            logger.info(f"[To {self._name}]{device}/{room} -> 조회")
        packet = (
        ...
        if v["command"] == "조회" and v["src_device"] == DEVICE_WALLPAD:
            ...
            logger.debug(
                f"[To {self._name}]{v['type']}({v['command']}) {v['src_device']}({v['src_room']}) -> {v['dst_device']}({v['dst_room']}) = {v['value']}"
            )
    ...
    def parse_switch(self, device, room, value="0000000000000000"):
        ...
        for i in range(1, to_i):
            ...
            if value[i * 2 - 2 : i * 2] != "00":
                on_count += 1
        switch[f"{device}0"] = "on" if on_count > 0 else "off"
        return switch
...
class Grex:
    ...
    def on_message(self, client, obj, msg):
        ...
        if "config" in _topic:
            ...
            logger.info(f"Message Fan: {msg.topic} = {_payload}")
    ...
    def homeassistant_device_discovery(self, initial=False):
        ...
        publish_list = []
        subscribe_list.append(("rs485/bridge/#", 0))
        ha_topic = f"{HA_PREFIX}/{HA_FAN}/grex_{DEVICE_FAN}/config"
        ha_payload = {
            "name": f"{self._name}_{DEVICE_FAN}",
        ...
        ha_payload = {
            "name": f"{self._name}_{DEVICE_FAN}_mode",
            "state_topic": f"{HA_PREFIX}/{HA_SENSOR}/grex_{DEVICE_FAN}/state",
            "state_value_template": f"{{{{ value_json.{DEVICE_FAN}_mode }}}}",
            "ic": "mdi:play-circle-outline",
            "uniq_id": f"{self._name}_grex_{DEVICE_FAN}_mode",
            "device": {
                "name": "Grex Ventilator",
                "ids": "grex_ventilator",
                "mf": "Grex",
        ...
        ha_payload = {
            "name": f"{self._name}_{DEVICE_FAN}_speed",
            "state_topic": f"{HA_PREFIX}/{HA_SENSOR}/grex_{DEVICE_FAN}/state",
            "state_value_template": f"{{{{ value_json.{DEVICE_FAN}_speed }}}}",
            "ic": "mdi:speedometer",
            "uniq_id": f"{self._name}_grex_{DEVICE_FAN}_speed",
            "device": {
                "name": "Grex Ventilator",
                "ids": "grex_ventilator",
                "mf": "Grex",
    ...
    def send_to_homeassistant(self, target, value):
        if target == HA_FAN:
            self.d_mqtt.publish(f"{HA_PREFIX}/{HA_FAN}/grex/state", json.dumps(value))
            logger.info(f"[To HA]{HA_PREFIX}/{HA_FAN}/grex/state = {json.dumps(value)}")
        elif target == HA_SENSOR:
            self.d_mqtt.publish(
                f"{HA_PREFIX}/{HA_SENSOR}/grex_{DEVICE_FAN}/state",
        ...
        while True:
            if ser.readable():
                ...
                if len(buf) >= packet_len:
                    ...
                    # logger.debug(f"[From {packet_name}]{joindata} {str(chksum[0])} {str(chksum[1]}")
                    if chksum[0]:
        ...
        if r._type == "serial":
            for device in r._device:
                if r._connect[device].isOpen():
                    ...
                    try:
                        logger.info(f"[CONFIG] {_name} 초기화")
                        if _name == "kocom":
